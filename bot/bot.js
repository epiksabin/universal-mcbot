...
function tryCreateBot(username, webhookUrl, retry = 0) {
  ...
  bot.on('kicked', (reason) => {
    const msg = `[✗] ${username} kicked: ${reason}`;
    console.log(msg);
  ...
  bot.on('error', (err) => {
    const msg = `[✗] ${username} error: ${err.message}`;
    console.log(msg);
...
function startSpamAndMovement(bot, username, chatLog) {
  console.log(`[+] ${username} started movement/chat`);
  chatLog.write(`[STARTED MOVEMENT + CHAT] ${new Date().toISOString()}\n`);
